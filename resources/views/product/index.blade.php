<x-admin-layout>
    ...
    <div class="container mx-auto px-4 sm:px-8">
        <div class="py-8">
            ...
            <div>
                <h2 class="text-2xl font-semibold leading-tight dark:text-gray-300">{{__('Products')}}</h2>
            </div>
                ...
                <div class="inline-block min-w-full shadow rounded-lg overflow-hidden">
                    <table class="min-w-full leading-normal">
                        ...
                        <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
                            ...
                            <th class="px-5 py-3 border-b-2 border-gray-200 dark:border-gray-500">{{__('SKU')}}</th>
                            <th class="px-5 py-3 border-b-2 border-gray-200 dark:border-gray-500">{{__('Price')}}</th>
                            <th class="px-5 py-3 border-b-2 border-gray-200 dark:border-gray-500">{{__('Stock')}}</th>
                                            ...
                                            <div class="flex items-center text-sm">
                                                ...
                                                <div class="relative hidden w-8 h-8 mr-3 rounded-full md:block">
                                                    <img class="object-cover w-full h-full rounded-full"
                                                         src="{{url('storage/products/'.$p->image)}}" alt=""
                                                         loading="lazy"/>
                                                    <div class="absolute inset-0 rounded-full shadow-inner" aria-hidden="true"></div>
                                                </div>
                                                <div>
                                                    <p class="font-semibold hover:text-gray-900 dark:hover:text-gray-200">{{$p->name}}</p>
                                                    <p class="text-xs text-gray-600 hover:text-gray-900 dark:text-gray-400">{{$p->category->name}}</p>
                                                </div>
                                            </div>
                                        </a>
                                    </td>
{{--                                    <td class="px-4 py-3 text-sm">{{($p->description)}}</td>--}}
                                    ...
                                    <td class="px-4 py-3 text-sm">R${{number_format($p->price,2,",",".")}}</td>
                                    <td class="px-4 py-3 text-sm">
                                        <div>
                                            <p class="text-xs text-gray-600 dark:text-gray-400">Min - {{$p->minimum_stock}}</p>
                                            <p class="text-xs text-gray-600 dark:text-gray-400">Max - {{$p->maximum_stock}}</p>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-4 py-3 text-sm">{{date_format(date_create($p->created_at), 'd/m/Y')}}</td>
                                    <td class="px-4 py-3">
                                        <div class="flex items-center space-x-4 text-sm">
                                            ...
                                            <button @click="openModal" onclick="mostrarDados({{$p->id}})" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="Delete">
                                                    <svg class="w-5 h-5" aria-hidden="true" fill="currentColor"
                                                         viewBox="0 0 20 20">
                    ...
                    <div class="px-5 py-5 bg-white dark:bg-gray-700 border-t dark:border-gray-600 flex flex-col xs:flex-row items-center xs:justify-between">
                        <caption>
                            {{ $products->appends(['filter' => isset($filter) ? $filter : '', 'search' => isset($search) ? $search : ''])->links() }}
                        </caption>
                    </div>
                </div>
            </div>
    ...
    <div
        ...
        <div
            ...
            <div class="mt-4 mb-6">
                <!-- Modal title -->
                <p class="mb-2 text-lg font-semibold text-gray-700 dark:text-gray-300">Delete Product</p>
                <!-- Modal description -->
